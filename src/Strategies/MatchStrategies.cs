...
{
    public bool CanApply(DynamicOptions options) => options.GetFlagValue("FixedStrings");

    public IEnumerable<MatchResult> FindMatches(string line, string pattern, DynamicOptions options, string fileName, int lineNumber)
    {
        var comparison = options.GetFlagValue("IgnoreCase") ? StringComparison.OrdinalIgnoreCase : StringComparison.Ordinal;
        var currentIndex = 0;
...
public class RegexMatchStrategy : IMatchStrategy
{
    ...
    public bool CanApply(DynamicOptions options) => options.GetFlagValue("ExtendedRegexp") || (!options.GetFlagValue("FixedStrings") && !options.GetFlagValue("WholeWord"));

    public IEnumerable<MatchResult> FindMatches(string line, string pattern, DynamicOptions options, string fileName, int lineNumber)
    {
        var regexOptions = RegexOptions.Compiled;
        if (options.GetFlagValue("IgnoreCase"))
...
{
    public bool CanApply(DynamicOptions options) => options.GetFlagValue("WholeWord");
