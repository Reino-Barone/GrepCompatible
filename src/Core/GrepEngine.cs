...
public interface IGrepEngine
{
    ...
    Task<SearchResult> SearchAsync(DynamicOptions options, CancellationToken cancellationToken = default);
}
...
public class ParallelGrepEngine(IMatchStrategyFactory strategyFactory) : IGrepEngine
{
    ...
    public async Task<SearchResult> SearchAsync(DynamicOptions options, CancellationToken cancellationToken = default)
    {
    ...
    private Task<IEnumerable<string>> ExpandRecursiveAsync(string path, DynamicOptions options, CancellationToken cancellationToken)
    {
        var files = new List<string>();
    ...
    private static bool ShouldIncludeFile(string filePath, DynamicOptions options)
    {
    ...
    private async Task<FileResult> ProcessFileAsync(string filePath, IMatchStrategy strategy, DynamicOptions options, CancellationToken cancellationToken)
    {
        try
        {
    ...
    private async Task<FileResult> ProcessStandardInputAsync(IMatchStrategy strategy, DynamicOptions options, CancellationToken cancellationToken)
    {
        var matches = new List<MatchResult>();
        var lineNumber = 0;
        var matchCount = 0;
